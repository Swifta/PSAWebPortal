...
public class BtnActionsClickListener implements ClickListener {
	...
	public void buttonClick(ClickEvent event) {
		...
		UI.getCurrent().getSession()
				.setAttribute(UserDetailsModule.SESSION_UDM_TABLE, arrID[1]);
		UI.getCurrent().getSession()
				.setAttribute(UserDetailsModule.SESSION_UDM_ID, arrID[2]);
		UI.getCurrent().getSession()
				.setAttribute(UserDetailsModule.SESSION_UDM_UNAME, arrID[3]);

		UI.getCurrent()
				.getSession()
				.setAttribute(
						UserDetailsModule.SESSION_UDM_ACTION_EDIT_DETAILS,
						arrID[4]);
